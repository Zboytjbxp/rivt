...
async function mapAlbumRow(database, row, { withPhotos = false } = {}) {
  const base = {
    id: row.id,
    name: row.name,
...
app.post("/api/v1/albums", requireV1AuthenticatedUser, requireV1Actor, writeRateLimit, asyncRoute(async (request, response) => {
  ...
  response.status(201).json({
    data: { album: await mapAlbumRow(database, { ...row.rows[0], photo_count: 0 }) },
    meta: { requestId: request.requestId },
  });
}));
...
app.get("/api/v1/albums/:id", requireV1AuthenticatedUser, requireV1Actor, asyncRoute(async (request, response) => {
  ...
  response.json({
    data: { album: await mapAlbumRow(database, row.rows[0], { withPhotos: true }) },
    meta: { requestId: request.requestId },
  });
}));
