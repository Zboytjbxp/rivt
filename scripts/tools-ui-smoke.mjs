...
async function runToolsFlow(page, viewportName) {
  ...
  assert.equal(await page.getByRole("button", { name: "Send", exact: true }).count(), 1, "invoice review should expose one compact send action");
  ...
  assert.equal(await page.locator(".v2-document-logo-fallback").count(), 0, "customer documents must not invent an initials logo");
  ...
  await page.getByRole("heading", { name: "Payment options to offer" }).waitFor({ timeout: 15_000 });
  const bankPaymentOption = page.getByRole("checkbox", { name: /Pay securely by bank/ });
  if (await bankPaymentOption.getAttribute("aria-checked") !== "true") await bankPaymentOption.click();
  ...
  await page.getByRole("button", { name: "Send", exact: true }).click();
  const invoiceSendSheet = page.getByRole("dialog", { name: "Send invoice" });
  await invoiceSendSheet.getByRole("button", { name: /^Email\b/ }).first().waitFor({ timeout: 15_000 });
  await invoiceSendSheet.getByRole("link", { name: /Text/ }).waitFor({ timeout: 15_000 });
  await invoiceSendSheet.getByRole("button", { name: /Email, then text/ }).waitFor({ timeout: 15_000 });
  await invoiceSendSheet.getByRole("button", { name: "Close send options" }).click();
  ...
  if (viewportName !== "se") {
    ...
    await page.getByRole("heading", { name: "Payment options to offer" }).waitFor({ timeout: 15_000 });
    const focusedBankPaymentOption = page.getByRole("checkbox", { name: /Pay securely by bank/ });
    if (await focusedBankPaymentOption.getAttribute("aria-checked") !== "true") await focusedBankPaymentOption.click();
