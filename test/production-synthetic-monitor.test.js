...
const issueHelperPath = join(repositoryRoot, ".github", "scripts", "production-synthetic-issue.cjs");
...
test("workflow artifacts and incident issues use the sanitized summary only", async () => {
  ...
  assert.doesNotMatch(workflow, /uses:\s*actions\/(?:checkout|setup-node|upload-artifact|github-script)@v\d+/);
  assert.match(workflow, /const allowedErrorCodes = new Set/);
  assert.equal(workflow.match(/findOwnedSyntheticIncident\(issues\)/g)?.length, 2);
  ...
  assert.doesNotMatch(workflow, /production-monitor\.log|Latest monitor output|```text/);
});

test("incident lookup ignores marker copies and accepts only the automation-owned issue", async () => {
  const imported = await import(pathToFileURL(issueHelperPath).href);
  const {
  ...
  const attackerIssue = {
    ...
    body: SYNTHETIC_INCIDENT_MARKER,
    user: { login: "untrusted-user", type: "User" },
  };
  const automationIssue = {
    number: 42,
    title: "Production synthetic check failing",
    body: `${SYNTHETIC_INCIDENT_MARKER}\n\nManaged by RIVT automation.`,
    repository_url: PRODUCTION_REPOSITORY_URL,
    user: { login: "github-actions[bot]", type: "Bot" },
  };

  const ineligibleVariants = [
    { ...automationIssue, number: 43, pull_request: { url: "https://example.invalid/pr/43" } },
    { ...automationIssue, number: 44, user: undefined },
    { ...automationIssue, number: 45, user: { login: "github-actions-bot", type: "Bot" } },
    { ...automationIssue, number: 46, user: { login: "github-actions[bot]", type: "User" } },
    { ...automationIssue, number: 47, title: "Lookalike synthetic issue" },
    { ...automationIssue, number: 48, body: "Marker removed" },
    { ...automationIssue, number: 49, body: `Copied text\n${SYNTHETIC_INCIDENT_MARKER}` },
    { ...automationIssue, number: 50, repository_url: "https://api.github.com/repos/other/repo" },
  ];

  assert.equal(findOwnedSyntheticIncident([attackerIssue]), undefined);
  assert.equal(findOwnedSyntheticIncident(ineligibleVariants), undefined);
  assert.equal(findOwnedSyntheticIncident([attackerIssue, automationIssue]), automationIssue);
});
