...
if (!testDatabaseUrl) {
  ...
  test("versioned migration lifecycle", async () => {
    ...
    try {
      ...
      assert.equal(baseline.pending.length, 19);
      ...
      assert.equal(applied.pending.length, 18);
      ...
      assert.equal(authFoundation.pending.length, 17);
      ...
      assert.equal(jobsFoundation.pending.length, 16);
      ...
      assert.equal(matchFoundation.pending.length, 15);
      ...
      assert.equal(messagingFoundation.pending.length, 14);
      ...
      assert.equal(projectFoundation.pending.length, 13);
      ...
      assert.equal(safetyFoundation.pending.length, 12);
      ...
      assert.equal(hardeningFoundation.pending.length, 11);
      ...
      assert.equal(shopTalkFoundation.pending.length, 10);
      ...
      assert.equal(shopTalkModeration.latestVersion, 20);
      ...
      assert.equal(reapplied.latestVersion, 20);
