...
## Packet 100A Source-Only Restart Proof Addendum - 2026-08-17

Packet 100A is accepted at its source-only restart-proof boundary on exact
candidate `14f0a6df7e445378d90bd62af7ef6812c0455694`. GitHub Actions push run
`32077077458` and PR run `32077080310` both passed build, repository lint, and
the aggregate test step: 544/544 unit tests and 30/30 integrations, including
six nested disposable-PostgreSQL cases for migration lifecycle, killed-process
reopen/reconciliation, expired-lease recovery with a higher fence and stale-
finalizer rejection, effect-before-response idempotence, concurrent exclusion,
and constraints/guarded rollback. Focused Packet/recovery/readiness tests pass
296/296. Final local source evidence separately passes all three browser E2E
journeys and a zero-vulnerability production dependency audit; CI skipped those
steps after readiness intentionally failed closed.

This advances only the providerless source evidence for `GA-OPS-004`. It
replaces an in-memory/simulated process-loss claim with real killed Node
subprocesses and durable restart through disposable PostgreSQL in exact-head CI.
The store requires the exact application, backup, and restore
database identities as prohibited identities and rechecks its exact injected
runtime database identity. It does not prove provider persistence, an
independently deployed failure domain, live forced termination, real writer-
policy retirement, or application-object recovery. Source capability remains
`in_progress_local_only`, writer-control evidence remains
`providerless-injected-fake`, and requirement maturity remains **Partial**.
The candidate is not merged, deployed, configured, scheduled, provider-proved,
or operationally approved.

Both exact-head runs stopped only on `ACTIVE_LAUNCH_HOLD`,
`RECURRING_BACKUP_INACTIVE`, `BACKUP_FRESHNESS_MONITOR_INACTIVE`,
`APPLICATION_OBJECT_RECOVERY_MISSING`, and
`RECOVERY_OPERATIONAL_APPROVAL_MISSING_OR_STALE`. These five launch blockers
remain unchanged.

## Operational Recovery Addendum - 2026-08-16 Active-Key-Only Restore

This addendum and the Admin and Operations requirements table are the current
recovery-readiness record. Earlier addenda and readiness statements below are
point-in-time history and are superseded where they conflict with this record.

- `GA-OPS-004` gains a fresh active-key-only PostgreSQL logical restore proof:
  109 tables and 8,862 rows restored into an isolated temporary database with
  zero row-count differences, zero content differences, 42 applied migrations,
  and zero pending migrations. Restore work took 2.721 seconds and verification
  took 51.235 seconds, for 53.956 seconds total against the approved 240-minute
  RTO target.
- `GA-OPS-005` gains least-lifetime credential and cleanup evidence: both
  temporary AWS writer and restore credentials are inactive; sensitive values
  in the temporary Railway vault are blank; the isolated restore database was
  dropped; the local PostgreSQL process and production tunnel were stopped;
  and temporary helpers were removed. A secret-safe comparison proved the
  active key matched the one-shot key and differed from the predecessor. After
  the predecessor configuration was blanked and a same-source redeploy
  succeeded, the running service reported the active key present and both
  previous aliases absent. In this 2026-08-16 addendum, no credential value or
  provider object coordinate is committed as evidence.
- `GA-OPS-008` gains post-closeout runtime evidence: public health returned the
  exact expected restore-hotfix source, the production monitor passed, and
  bank payments remained disabled.
- Requirement maturity remains **Partial**. The explicit launch hold is still
  active, recurring backup scheduling is still inactive, and PostgreSQL-only
...
- Packet 100/100A source passes 296/296 focused Packet/recovery/readiness tests,
  544/544 unit tests, and 30/30 exact-head CI integrations; repository security
  lint, JSON, and diff integrity also pass. Its
  exact-key create and retention-read grants share the same half-open UTC
  window, with `If-None-Match` limited to creates, and the retained correction
  remains covered by 10/10 focused writer-policy tests. The preceding source
  passed build, repository lint, and aggregate tests in both exact-head CI
  runs. Final local source evidence passed all browser journeys and the zero-
  vulnerability production dependency audit; CI skipped those gates after the
  intentional readiness failure. The guarded local
  harness remains providerless and reports no provider I/O, production-data
  read, or charge-bearing action. The
  provider-neutral writer lease now durably records the exact provider plan
  before opening any adapter factory, registers revocation before activation,
  retires attempted authority on every later in-process failure, and requires
  verified inertness before final evidence. The selected Option 2 source also
  makes an independent-controller registration durable before writer
  construction. Its v2 reservation and final v5 restricted receipt bind
  distinct control/auditor identity digests, only the digest of the controller-
  private retirement descriptor, registration, and retirement finalization;
  CAS revisions, fencing tokens, bounded attempt leases, deadline
  sweeps, conflict quarantine, and retry reconcile ambiguous registration and
  controller-process loss. Packet 100A proves killed-process persistence and
  restart against disposable PostgreSQL in exact-head CI. Source-only writer-
  control evidence is fixed to
  `providerless-injected-fake`. The concrete AWS provider control adapter,
  provider-derived writer/account/bucket-owner/control/auditor bindings,
  independently deployed controller, provider persistence, live forced-
  termination evidence, and live control/data-plane proof remain open. A
  providerless isolated route reader now proves the production upload-URL
  handler with injected stores,
  ...
  while readiness requires `live-cookie-session-http-provider-delivery`, so
  requirement maturity does not advance.

## Operational Incident Addendum - 2026-07-29 Credential Containment

- Historical source-preparation evidence for `GA-OPS-004` added unactivated
  backup-only hardening on
  ...
  activation, or complete disaster recovery. At that point the backup-key
  recurrence proof and application-object recovery were both pending; the
  2026-08-16 addendum supersedes that point-in-time status.
...
| GA-OPS-004 | Backup and timed restore drill pass | Partial | On 2026-08-16, one active-key-only PostgreSQL logical artifact restored into an isolated temporary database: 109 tables, 8,862 rows, 42 applied migrations, zero pending migrations, zero count diffs, and zero content diffs. Restore plus verification completed in 53.956 seconds against the 240-minute PostgreSQL logical-recovery target; the target and temporary access paths were removed afterward. Packet 100 adds source-only coordinated database-plus-application-object recovery tooling: one database/object mutation barrier, independently authenticated encrypted archive members, an exact three-key writer plan, three exact-version reader grants, durable exact-version/hash/retention evidence, and local adversarial tests. Its provider-neutral writer lease registers revocation before activation, retires attempted authority on every later failure, and requires verified inertness before final evidence. Selected Option 2 source also requires durable independent-controller registration before writer construction and binds its locally reconciled, fenced retirement lifecycle in a v2 reservation and final v5 restricted receipt. Packet 100A proves durable reopen and reconciliation after real killed Node subprocesses against disposable PostgreSQL in exact-head CI, with the exact application, backup, and restore database identities prohibited. This remains isolated providerless source proof: provider persistence, an independently deployed controller, and live forced-termination evidence remain missing. Providerless evidence cannot satisfy the live writer-authority readiness guard. The create command remains inert without the concrete AWS control adapter and independently deployed controller. Nonempty restore has a providerless isolated reader that rechecks target identity in a read-only transaction, invokes the production upload-URL handler for one exact owner/reference per completed scope, and hashes handler-selected bytes from an injected target store. That is handler-level source proof, not cookie/session middleware, live HTTP/provider delivery, or coordinated restore evidence. No provider I/O, production-data read, merge, deployment, schedule, or cost occurred. This does not advance the requirement: the next PostgreSQL drill is due 2026-09-15, recurring backup is inactive, independent freshness monitoring is inactive, application-object-byte recovery is missing, and current operational approval is pending. Full disaster recovery and launch readiness are not proved. |
...
- Historical founder/support/legal-safety signoff and incident-routing evidence
  remain recorded. Current launch readiness is **blocked** by the explicit
  `ACTIVE_LAUNCH_HOLD` and by the recovery policy's operational-readiness
  state: recurring backup and independent backup-freshness monitoring are
  inactive, application-object-byte recovery is missing, and a new operational
  approval is pending. Earlier `ready` outputs in this file are point-in-time
  evidence and do not override the current machine-readable state.
