...
| GA-OPS-004 | Backup and timed restore drill pass | Blocker | Encrypted logical-backup and isolated-restore tooling exists, the prior named PostgreSQL restore passed, and the explicitly authorized 2026-07-29 artifact is inside the approved 24-hour RPO; `launch:readiness -- --require-ready` now passes. The fresh artifact itself has not been restored, and it records object keys/metadata rather than object bytes, so photos, attachments, logos, and evidence have no demonstrated byte recovery. Exit requires an isolated restore from this fresh artifact, representative object-byte restore evidence, and scheduled/monitored recurrence approved by the owner. |
