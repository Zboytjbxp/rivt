...
| GA-OPS-009 | Hosting capacity, redundancy, failover, and migration decision are evidenced | Blocker | Packet 92 binds the exact target/live-source distinction. Packet 88 observed Railway Hobby with one app, one PostgreSQL service, and one region; Michael later supplied evidence that Pro is active, which satisfies the plan prerequisite but does not prove a topology or resilience change. HTTP, migrations, push delivery, maintenance, and a default 10-connection pool currently share each process, so replicas require explicit web/worker/migrate roles and connection/memory budgets first. The selected design keeps Railway, proposes a 99.9% product SLO and measured migration triggers, and defines cost-capped two-replica, HA/PITR, webhook-safe edge, load, failover, recovery, monitoring, rollback, and AWS ECS Express/RDS exit gates. The repository build/lint, 173 unit tests, 24 integration suites, four E2E scripts, dependency audit, and hardening artifact checks pass. Current `launch:readiness` is not sufficient because it does not evaluate this new requirement. No traffic/resource baseline, numeric approved SLO, load/failover result, HA/failure-domain contract, provider quote for the hardened topology, further owner cost approval, Codex provider mutation, or deployment exists. Exit requires a privacy-safe seven-day baseline, approved SLO/cost policy, replica-safe source, exact deployed two-web/worker topology, bounded connection/upload budgets, documented and twice-tested PostgreSQL failover, isolated PITR and independent restore, webhook-safe edge proof, pilot plus 10× load evidence, alerts, automated readiness enforcement, and rehearsed rollback. |
