...
- `GA-OPS-008` gains exact-source production evidence from Railway deployment
  ...
  created. Completion still requires owner-device re-registration, an
  explicitly sent and received/tapped physical test alert, and a post-test
  inventory showing current-generation success for every eligible device with
  zero previous/unknown/unrecognized dependencies. The older Railway Stage 1
  worker branch must be rebased and re-reviewed so its success path preserves
  this generation evidence before any new Stage 1 approval.
