...
- `GA-OPS-009` remains **Blocker** with no maturity promotion. Packet 94 adds
  role-specific Railway web/worker config, role-bound direct Node starts,
  web-only advisory-locked schema migration that rejects worker/standalone
  ownership, a non-mutating worker readiness
  pre-deploy command, 30-second provider drain, and a PostgreSQL effective
  connection-ceiling check across global, current-database, and current-role
  limits. A hosted worker validates required push delivery before any
  delivery controller can claim work, enforces a bounded outbound deadline,
  and validates a nominal post-request scheduling margin inside the
  provider drain. Forced shutdown remains the actual bound if database work
  stalls; guaranteed cleanup is not claimed. These are activation
  prerequisites, not deployed topology, capacity, HA, failover, PITR, edge,
  load, cost, or rollback evidence.
- The future rollout is deliberately staged and manual: deploy web first,
  retain its migration and readiness evidence, then start one worker from the
  same candidate source. Git-triggered services are not treated as an ordered
  transaction. After Stage 1 proof, a separate cost/config approval is
  required before adding a second same-region web replica. The runbook
  requires actual/declarative replica reconciliation, transition and
  steady-state database budgets with at least 30% headroom, a fresh recovery
  checkpoint, exact source identity, an approved cost ceiling, worker
  evidence, session/webhook/push/upload checks, and non-destructive rollback.
- The preflight linter binds a redacted proposed activation plan to a fresh
  read-only provider snapshot and local source/config facts. Passing proves
  only pre-activation consistency, not that activation or runtime acceptance
  occurred. The separate evidence schema remains empty until a future
  authorized activation records actual results.
- A bounded read-only Railway CLI observation confirms the live source remains
  `92a8451` on one app service plus one PostgreSQL service in `us-east4`,
  with no worker and with the old app config still effective. A separate
...
- `GA-OPS-007` remains **Partial**. Focused local role, readiness, push, and
  entrypoint tests do not replace the full repository, PostgreSQL,
  browser/manual, or exact-runtime acceptance gates.
- `GA-OPS-008` remains **Partial**. Packet 94 used bounded read-only provider
  status access without reading variables, credentials, or production data.
  It created no service or replica, changed no environment, incurred no
  incremental cost, merged nothing, and deployed nothing.
- `R-054` and `R-055` remain High/open; `R-056` remains Medium/open. Railway
  Under Attack Mode is explicitly not accepted as an always-on WAF because it
  can block Stripe webhooks and other machine clients. Private PostgreSQL
  networking is required, but public-client certificate authentication
  remains unproved. A provider Compute hard limit is not accepted without its
  exact workspace scope and outage behavior; estimates/alerts are not hard
  caps, and an unavailable enforced cap requires explicit residual-risk and
  manual-stop approval.
- Implementation commit
  `e5d952ca454a2857c131e1b860ad9cd07dc6399a` passed build,
  application/security lint, 39/39 focused role/push/preflight/entrypoint
  tests, 222/222 unit/frontend tests, 27/27 serial PostgreSQL integration
...
- `GA-OPS-009` remains **Blocker** with no maturity promotion. Packet 93
  implements explicit hosted `web`, `worker`, and `migrate` roles, a
  local/test-only `combined` role, role-specific PostgreSQL pools, and a
  fail-closed connection budget with at least 30% declared headroom. These
  source controls remove a prerequisite hazard; they do not prove deployed
  replicas, capacity, HA, failover, PITR, edge controls, load, or rollback.
- Web and worker startup use a non-mutating migration-ledger assertion;
  migration application remains confined to the one-shot migration role under
  the checksummed advisory-locked migration system. Worker maintenance gains
  a PostgreSQL lease, push finalization is fenced to the claimed attempt, and
  shutdown awaits active worker/maintenance work.
- `GA-OPS-005` remains **Partial**. Packet 93 adds a disabled-by-default,
  fixed-schema aggregate capacity signal for allowlisted route families,
  latency buckets and process/pool/upload/worker/maintenance/dependency
  measures. Exact shutdown outcome/duration remains in bounded coordinator
  events. The capacity stream excludes raw paths, query strings, actor IDs,
  and arbitrary free text, but no deployed retention, seven-day baseline,
  alert, or SLO evidence exists.
- `GA-OPS-008` remains **Partial**. The packet changes no Railway service,
  replica, database, provider setting, environment, monitoring vendor,
  production data, or deployed source.
- `GA-OPS-007` gains exact local evidence from implementation commit
  `234f73455e2283d20ef06c09077123ee1afa61d1`: build; application/security
  lint; 79/79 focused runtime/security tests; 204/204 unit/frontend tests;
  26/26 PostgreSQL integration suites in 1,489.1 seconds; authentication
  fail-closed, jobs/discovery desktop+mobile, offline-recovery, and production
  CSP E2E paths; a zero-vulnerability production dependency audit; and diff
  integrity. Branch `codex/railway-replica-safety` was pushed through final
  tip `037c5a0`; it has not been merged or deployed.
- `R-054` remains High/open and `R-056` remains Medium/open. Any staged
  provider topology, generated load, failover operation, or incremental cost
...
- `GA-OPS-004` remains **Blocker** with no maturity promotion. Michael
  selected the independent cross-provider immutable-backup target, and Packet
  90 implements the provider-neutral local foundation only. Logical artifact
  v2 now captures one canonical repeatable-read snapshot, preserves exact
  PostgreSQL values/types, and verifies encrypted per-table/database content
  digests before a restore may commit.
- The local object harness requires explicit physical source binding,
  enforces bounded streaming AES-256-GCM encryption and digest/size checks,
  uses opaque destination names, and writes an authenticated encrypted
  completion object last. Its guarded fixtures prove no provider I/O,
  production-data read, or charge-bearing action occurred.
- This evidence is not an independent backup or production restore. The exact
  fresh v1 artifact remains un-restored, production object bytes were not
  read/copied, legacy objects without trustworthy hashes remain incomplete,
  and provider/account/region, DPA, retention/deletion, key custody, cost,
  scheduling, monitoring, and immutable-provider proof remain open.
  `R-051` therefore remains Critical and public launch remains blocked on its
  recorded exit criteria.
- `GA-OPS-007` gains local verification from production build,
  application/security lint, 173/173 unit/frontend tests, 24/24 PostgreSQL
  integration tests with zero skips, all four E2E paths, incident/launch
  readiness, hardening JSON validation, diff integrity, zero reported
  production dependency vulnerabilities, and guarded local recovery-runner
  evidence. The unit and integration commands are the exact components of
  `npm run test`; the 23-minute integration leg was not rerun solely to
  duplicate the aggregate wrapper.
- `GA-OPS-008` remains **Partial**. Packet 90 changed no provider resource,
  production configuration, database, bucket, DNS, plan, or deployed source.
...
- `GA-FND-005` gains a local bounded-input layer for JSON, URL-encoded,
  multipart-file, multipart-field, part, and header inputs. This does not
  change its **Partial** maturity because legacy API validation remains.
- `GA-OPS-004` moves from **Verified** to **Blocker** for current launch
  approval. A fresh encrypted logical artifact is now inside the approved
  24-hour RPO, but that exact artifact has not been restored and the logical
  format restores PostgreSQL records plus object keys/metadata rather than the
  object bytes.
- `GA-OPS-005` remains **Partial**. Local HTTP timeout/drain, cached bounded
  PostgreSQL/object-storage liveness probes, session-security readiness, and
  redaction improvements do not prove account-level resilience. Read-only
  Railway inspection confirms the Hobby plan, one application replica plus
  one PostgreSQL replica in `us-east4`, WAF available with Under Attack
  disabled, and CDN available but disabled. Under Attack cannot be blindly
  enabled because non-browser webhooks/API clients can be blocked.
- `GA-OPS-006` gains local pre-parse burst limiting, dual login subjects,
  stricter request/header/multipart bounds, and bounded non-blocking draining
  of expired durable-limiter rows. Existing durable PostgreSQL limits remain
  the authoritative cross-instance control. No edge-provider protection is
  claimed.
- `GA-OPS-007` gains final exact-worktree local evidence: production build,
  repository lint, security lint, 150/150 unit/frontend checks, the full
  PostgreSQL-backed aggregate, four E2E paths, all four required rendered UI
  smokes, incident/launch readiness, zero reported production dependency
  vulnerabilities, diff integrity, and a scoped current-source
  credential-pattern scan with no reported matches.
- `GA-OPS-008` remains **Partial**. Packet 88 has not been deployed and no
  provider configuration, DNS, database, canonical application data, or
  recurring/cost-bearing infrastructure action is represented as complete.
  The only object-storage write was the one fresh encrypted backup explicitly
  authorized for this packet.
- Public launch remains blocked on restoring the fresh artifact,
  object-byte recovery, malware scanning/quarantine,
  retention/DSAR/export/deletion, provider and legal approval, edge WAF
  evidence, redundancy/HA/autoscale evidence, and certificate-authenticated
  TLS for any public database backup/restore path. The normal production
  application path is verified as Railway's private internal `DATABASE_URL`.
- This is engineering evidence only. It does not establish SOC 2, PCI,
  privacy-law, penetration-test, availability, or other certification.
...
| GA-OPS-002 | Direct production dependencies are declared and vulnerability gate passes | Verified | `fast-xml-parser` is direct, Multer is 2.2.0, all direct dependencies/devDependencies are exact-pinned in local source, `.nvmrc` selects Node 22, `engines.node` is `>=22 <25`, and the Packet 88 exact-worktree `npm audit --omit=dev` reports zero production vulnerabilities. |
...
| GA-OPS-004 | Backup and timed restore drill pass | Blocker | Encrypted logical-backup and isolated-restore tooling exists, the prior named PostgreSQL restore passed, and the explicitly authorized 2026-07-29 artifact is inside the approved 24-hour RPO. Packet 90 adds a lossless, content-digested v2 logical format and a bounded provider-neutral encrypted object harness, but these are local engineering controls only. The exact fresh v1 artifact has not been restored, production object bytes have not been copied or recovered from an independent retained backup, and no provider immutability or scheduled recurrence has been proved. Exit still requires an isolated restore from the exact fresh artifact, complete independently retained object-byte restore evidence with integrity proof, and owner-approved scheduled/monitored recurrence. |
...
| GA-OPS-009 | Hosting capacity, redundancy, failover, and migration decision are evidenced | Blocker | Packet 92 binds the exact target/live-source distinction and selects hardened Railway with measured AWS exit triggers. Michael supplied evidence that Pro is active, which satisfies the plan prerequisite but does not prove topology or resilience. Packet 93 implementation commit `234f73455e2283d20ef06c09077123ee1afa61d1` adds source-only replica safety: explicit fail-closed hosted web/worker/migrate roles, local/test-only combined execution, non-mutating web/worker migration checks, role-specific pools, a declared connection budget retaining at least 30% headroom, leased maintenance, attempt-fenced push finalization, drainable workers, and fixed-schema aggregate capacity telemetry. Its build, lint, focused tests, 204 unit/frontend tests, 26 integration suites, four E2E paths, dependency audit, and diff integrity pass locally; branch `codex/railway-replica-safety` was pushed through final tip `037c5a0`, but none of this branch-only behavior is merged or deployed. Packet 94 implementation commit `e5d952ca454a2857c131e1b860ad9cd07dc6399a` adds role-bound Railway starts, web-only advisory-locked schema migration, a non-mutating worker pre-deploy readiness check, a 30-second provider drain, startup verification against the effective minimum of PostgreSQL global/database/role limits, mandatory push readiness for a hosted worker, a bounded push deadline, and a shutdown budget with a nominal post-request scheduling margin and an explicit forced-shutdown bound. Its no-cost runbook requires exact provider facts, a staged quote and cost ceiling, manually ordered web-first then worker activation from one source, a separately approved second web replica, transition/steady-state headroom, a plan bound to fresh read-only provider/local facts without confusing consistency for activation evidence, exact hard-limit scope/outage behavior, and non-destructive rollback. It is local and undeployed. `GA-OPS-007` and `GA-OPS-008` remain Partial, and current `launch:readiness` is not sufficient because it does not evaluate this requirement. No seven-day traffic/resource baseline, approved numeric SLO/cost ceiling, deployed multi-process topology, load/failover result, HA/failure-domain contract, hardened-topology quote, further owner cost approval, provider mutation, or deployment exists. Exit requires a privacy-safe seven-day baseline, approved SLO/cost policy, verified and exact deployed two-web/worker topology, bounded connection/upload budgets, documented and twice-tested PostgreSQL failover, isolated PITR and independent restore, webhook-safe edge proof, pilot plus 10x load evidence, alerts, automated readiness enforcement, and rehearsed rollback. |
