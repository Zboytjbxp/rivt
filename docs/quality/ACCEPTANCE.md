...
- Launch readiness gate: `docs/operations/recovery-policy.json`, `docs/operations/LAUNCH_OPS_CHECKLIST.md`, `docs/operations/INCIDENT_REHEARSAL_RUNBOOK.md`, and `npm run launch:readiness` now combine incident readiness with recovery-policy evidence. The recovery policy is approved for Gate A with RPO 24 hours, RTO 4 hours, 30-day backup retention, 30-day restore-drill cadence, and next restore drill due `2026-07-21T04:18:59.000Z`. The current gate remains blocked by founder/support/legal-safety approvals and deeper manual accessibility/device evidence.
- Approval packet: `docs/operations/GATE_A_APPROVAL_PACKET.md` now summarizes the final founder/support/legal-safety approval scopes, evidence, known risk acceptance, and exact signoff fields. It is not an approval; `docs/operations/incident-routing.json` remains the machine-readable source of approval status.
...
- Final founder/support/legal-safety approvals. `npm run incident:readiness -- --require-ready` must pass before launch.
...
- Founder/legal/support approval fields, launch communications, and physical/deeper manual accessibility-device evidence.
