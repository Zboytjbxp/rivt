...
- Backup-artifact restore and RPO/RTO acceptance if Gate A requires proof from a specific backup object. The timed isolated logical restore is complete, but it restored from the live source database rather than a named backup artifact.
- Dedicated error monitoring, alert rules, paging/incident owner routing, and one incident rehearsal. The scheduled GitHub synthetic check plus incident issue loop is only a first external tripwire.
