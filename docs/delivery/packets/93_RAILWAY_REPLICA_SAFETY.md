...
- PostgreSQL pool acquisition and idle-client lifetimes are bounded, but
  ...
  `rejectUnauthorized: false` path. `R-056` remains open until an exact
...
- `R-055` remains High/open. Replica-safe source is one exit prerequisite;
...
- `R-057` remains Medium/open. The new capacity format avoids arbitrary free
