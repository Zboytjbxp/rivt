...
| R-002 | Critical | Private records are session-cookie scoped without authenticated ownership | Mitigated and CI-proven for legacy routes with disposable-DB cross-user isolation | Deploy, live-smoke, then normalize domain ownership |
...
| R-005 | High | Session fixation/overlong sessions | Mitigated and CI-proven with DB-backed session rotation | Deploy/live-smoke and later add device/session controls |
