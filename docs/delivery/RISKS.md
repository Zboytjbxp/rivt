...
| R-006 | High | Core records can conflict or be overwritten as one JSON blob | Mitigated for jobs in Packet 03 with normalized tables, optimistic versioning, and idempotent writes; applications/messages/records remain app-state work | Normalize each remaining domain and remove whole-state authority |
