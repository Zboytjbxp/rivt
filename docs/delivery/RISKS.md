...
| R-003 | Critical | Seed jobs/talent/reviews can appear as real marketplace activity | Mitigated in Packet 03 source: `src/data.ts` exports no jobs or talent, Work/Crew use empty states, and E2E asserts seeded names are absent. Dead legacy fixtures remain isolated from rendered production routes until the App strangler finishes. | Production empty states; synthetic data confined to tests/demo environment and legacy fixtures deleted before Gate A launch |
...
| R-015 | Medium | Navigation contradicts approved IA | Mitigated in Packet 03 source: primary shell is Home, Work, Crew, Shop Talk, Tools; messages moved to top-bar action and local E2E covers desktop/mobile | Verify in CI and deployed release |
