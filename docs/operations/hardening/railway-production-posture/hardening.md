...
- independent database-plus-object recovery (`R-052`);
- malware scanning/quarantine (`R-053`);
- data retention/DSAR/provider governance (`R-054`);
- edge, redundancy, HA, autoscaling, load, and failover (`R-055`);
- public database TLS verification for non-private clients (`R-056`);
