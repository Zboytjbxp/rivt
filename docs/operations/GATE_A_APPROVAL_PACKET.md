...
payment approval but now fails closed on `ACTIVE_LAUNCH_HOLD`, the unverified
private backup-contact route, incident approvals that predate the latest
rehearsal evidence, and unproved continuous-recovery controls. Fresh incident
approvals must be recorded only after the private backup route is verified; use
...
- Backup owner role: `backup-incident-owner`. Its access-controlled contact route and successful route-test evidence are not yet recorded, so incident readiness must remain blocked.
...
> I approve the Gate A support plan for the named-cohort pilot: support@rivt.pro, Monday-Saturday, 9:00 AM-5:00 PM America/New_York, with the founder as primary support owner and the `backup-incident-owner` role as backup.
...
- RIVT should keep the pilot small enough that the founder can personally support and observe it.

## Exact Approval Command

If the founder approves all three final signoffs, record them in `docs/operations/incident-routing.json`:

- `approvals.founder.status = "approved"`
- `approvals.support.status = "approved"`
- `approvals.legalSafety.status = "approved"`
- `approverRoleId = "founder"`, `"support-owner"`, or `"legal-safety-owner"` as applicable
- `approvedAt = current UTC timestamp`, recorded only after the latest passed
  rehearsal and successful private backup-route test
- Set those approval fields first, run
  `npm run incident:readiness -- --json`, then copy
  `summary.requiredApprovalConfigurationDigest` into each approval's
  `configurationDigest`. All three approvals must bind the same exact plan.
- If support coverage, owners/routes, providers, alert destinations, route
  approval, rehearsals, hold state, or another incident-plan detail changes,
  record new approvals and a new matching digest.
