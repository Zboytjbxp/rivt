...
> **Expired historical planning snapshot.** The approval window associated
> with this dated brief has expired and cannot be reused after the production
> credential incident and release-candidate source changes. This document does
> not authorize activation. A future provider action requires incident closure,
> a fresh provider snapshot, the final exact candidate and rollback SHAs, a new
> cost/outage statement, and Michael's explicit approval of that exact record.
...
approves a fresh, exact cost and outage statement. Any future approval expires
after 24 hours and must bind the exact candidate source and rollback target.
